...
DifficultySelectionMenu::DifficultySelectionMenu( SDL_Renderer* renderer, Game* game, TextureManager* textureManager, rapidjson::Value* json )
{
    ...
    *hard = (*json)["difficulties"]["hard"].GetFloat();
    ...
    buttons.push_back({
        Button( textureManager->GetButtonTexture( "button1" ), easyButtonPos, renderer, (void*)easy, []( Game* game, float* number ){ game->ChangeEnemyLevelMultiplier( *number ); } ),
    ...
    buttons.push_back({
        Button( textureManager->GetButtonTexture( "button1" ), mediumButtonPos, renderer, (void*)medium, []( Game* game, float* number ){ game->ChangeEnemyLevelMultiplier( *number ); } ),
    ...
    buttons.push_back({
        Button( textureManager->GetButtonTexture( "button1" ), hardButtonPos, renderer, (void*)hard, []( Game* game, float* number ){ game->ChangeEnemyLevelMultiplier( *number ); } ),
...
void DifficultySelectionMenu::HandleEvents( SDL_Event* event )
{
    for( auto& button : buttons )
    {
        if( button.HandleEvents( event ) )
            button.button.game_numbered( game, (float*)button.button.GetArg() );
    }
}